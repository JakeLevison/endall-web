...
//
// Company name is dynamic: if the user filled the demo gate, we use their
// company name. Otherwise we fall back to "your company". The static XLSX
// files still contain the sample data, but the chat text personalizes.
...
function getDemoCompany(): string {
  ...
  try {
    return localStorage.getItem("endall_demo_company") || "your company";
  } catch {
    return "your company";
  }
}
...
function getDemoCompanySlug(): string {
  ...
  if (name === "your company") return "Sample_Company";
...
const RAW_PRESETS: Record<string, DemoPreset> = {
  financial_model: {
    ...
    files: [
      {
        filename: `${PLACEHOLDER}_FinancialModel.xlsx`,
  ...
  npv_analysis: {
    ...
    files: [
      {
        filename: `${PLACEHOLDER}_NPV.xlsx`,
  ...
  competitive_analysis: {
    ...
    files: [
      {
        filename: `${PLACEHOLDER}_CompetitiveAnalysis.pdf`,
  ...
  review_financials: {
    ...
    files: [
      {
        filename: `${PLACEHOLDER}_FinancialReview.pdf`,
